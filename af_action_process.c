...
compose_instance(AF_INSTANCE* instance, char* raw_instance)
{
	...
	strcat(raw_instance,instance->invoke);strcat(raw_instance,"|");
	...
	strcat(raw_instance,temp_buff);strcat(raw_instance,"|");
	sprintf(temp_buff, "%d", instance->value);
	strcat(raw_instance,temp_buff);strcat(raw_instance,"|");
...
extract_raw_data(AF_INSTANCE* instance, EQX_MSG* emsg, int* eventType, char *err)
{
	...
	if(strcmp(emsg->raw[0].type,"request")==0&&strstr(emsg->raw[0].orig,".ES05.0.")!=NULL){
	//SFLOG_DEBUG("case request");
		*eventType = MY_AF_EVENT_CLIENT;
		strcpy(instance->incoming_addr, emsg->raw[0].orig);
		strcpy(instance->invoke, emsg->raw[0].invoke);
		char s[10],*p;
		strcpy(s,emsg->raw[0].data);
		if(strlen(s)>1){
			instance->value=s[1]-'0';
		}else{
			instance->value=0;
		}
	}else if(strcmp(emsg->raw[0].type,"response")==0){
	SFLOG_DEBUG("case response");
		*eventType = MY_AF_EVENT_DB;
	}else if(strcmp(emsg->raw[0].type,"request")==0&&strstr(emsg->raw[0].orig,".ES05.1.")!=NULL){
		*eventType = MY_AF_EVENT_DB2;
	}
	return 0;
}
...
{
	if (instance->command == MY_AF_SEND_IDLE)
	{
		...
		char c=emsg->raw[0].data[0];
		int result=0;
		switch(c){
			case '+':
				result=instance->value+instance->value;
				break;
			case '-':
				result=instance->value-instance->value;
				break;
		...
		sprintf(emsg->raw[0].data, "%d", result);
		//strcpy(emsg->raw[0].data,"test");
		SFLOG_DEBUG("Construct Idle");
	}else if (instance->command == MY_AF_SEND_DB){
		sprintf(emsg->raw[0].to, "BDF.ES04.DB");
		sprintf(emsg->raw[0].udef[UDEF_ATTR_URL], "/10.138.46.216:9000");
		sprintf(emsg->raw[0].type, "%s", "request");
		strcpy(emsg->raw[0].data,"");
		SFLOG_DEBUG("Construct DB");
	}else if (instance->command == MY_AF_SEND_DB2){
		sprintf(emsg->raw[0].to, emsg->raw[0].orig);
		//sprintf(emsg->raw[0].udef[UDEF_ATTR_URL], "/10.138.46.216:9000");
		sprintf(emsg->raw[0].type, "%s", "response");
		strcpy(emsg->raw[0].data,"success");
		SFLOG_DEBUG("Construct DB2");
	}
	return 0;
}
...
af_action_process(EC_UTILS* utils, EQX_MSG* emsg, int data_entry, char* state, char* err)
{
	...
	SFLOG_DEBUG("Ret = %d",af_return_code);
	/*
	...
	SFLOG_DEBUG("instance= %s",instance->incoming_addr);
	...
	if(af_return_code == MY_AF_RET_NORMAL) {
		...
		switch(af_state) {
			case MY_AF_STATE_IDLE:
				...
				break;
			case MY_AF_STATE_W_DB:
				af_next_state = af_state_w_db(messageEvent, utils, emsg, instance, err);
				break;
			default:
		...
		//SFLOG_DEBUG("Returncode= %d",af_return_code);
		SFLOG_DEBUG("******************************************************************************************");
	...
	SFLOG_DEBUG("Next = %d",af_next_state);
